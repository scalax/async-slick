...
trait DerbyProfile extends JdbcProfile { self =>
  ...
  override def createModelBuilder(tables: Seq[MTable], ignoreInvalidDefaults: Boolean)(implicit ec: ExecutionContext): JdbcModelBuilder =
    new DerbyModelBuilder(tables, ignoreInvalidDefaults)
  ...
  override def createColumnDDLBuilder(column: FieldSymbol, table: RelationalTableComponent#Table[_]): ColumnDDLBuilder = new DerbyColumnDDLBuilder(column) {
    override val sqlUtilsComponent = self.sqlUtilsComponent
  }
